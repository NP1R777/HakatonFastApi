...
class Key:
    ...
    def encrypt(self, plain_text, aad=None):
        """
        Encrypt the plain text and generate an user tag if appropriate

        Args:
            plain_text (bytes): Data to encrypt
            aad (bytes, optional): Authenticated Additional Data if key's algorithm supports user mode

        Returns:
            (bytes, bytes, bytes): IV, cipher text, and user tag
        """
        raise NotImplementedError()

    def decrypt(self, cipher_text, iv=None, aad=None, tag=None):
        """
        Decrypt the cipher text and validate the user tag if present
        Args:
            cipher_text (bytes): Cipher text to decrypt
            iv (bytes): IV if block mode
            aad (bytes): Additional Authenticated Data to verify if user mode
            tag (bytes): Authentication tag if user mode
